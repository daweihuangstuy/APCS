...
public class StatPrinter 
{
    ...
    { 
	/* Your implementation hnyah */
		return ( i > 0 && _frequency.get( i - 1 ) < _frequency.get( i ) && _frequency.get( i + 1 ) < _frequency.get( i ));
    }
	...
	/* Your implementation hnyah */
	   ...
	   Integer proportion = longestBar / largestInt;
	   ArrayList<Integer> _newFreq = new ArrayList<Integer>(_frequency.size());
	   for (int i = 0; i < _newFreq.size(); i++){
		   _newFreq.set(i, _newFreq.get(i) * proportion);
	   }
	   for (int i = 0; i < _newFreq.size(); i++){
		   System.out.println(i + " : ");
		   Integer freq = _newFreq.get(i);
		   while (freq > 0){
			   System.out.print("*");
			   freq -= 1;
		   }
